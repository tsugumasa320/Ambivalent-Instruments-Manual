...
VCV Rack向けのリアルタイム・グラニュラーシンセシス・プロセッサーです。
...
本モジュールは Max for Live 版 grainer をベースにしています: [https://tsugumasa320.gumroad.com/l/grainer](https://tsugumasa320.gumroad.com/l/grainer)
...
- **Forward/Reverse**: 再生方向切り替え（ボタン）
- **Freeze**: バッファ内容の固定（ボタン/CVゲート）

### CV入力

主要パラメータはCV制御に対応（±5V バイポーラ）:

- Grain Length CV
- Density CV
- Pitch CV（コンテキストメニューで1V/octオプション）
- Pitch Random CV
- Position CV
- Position Random CV
- Stereo Spread CV
...
- **バッファサイズ**: 5秒循環オーディオバッファ（44.1kHzにて）
- **グレイン数**: 最大64同時グレイン（初期値32）
- **エンベロープ**: Hannウィンドウによる自然なグレイン遷移
- **サンプルレート**: VCV Rack全サンプルレートに対応
- **オーディオ範囲**: ±5V VCV Rack標準