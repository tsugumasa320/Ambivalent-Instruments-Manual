...
`Feedback EQ8` は `EQ8` と同じ固定バンド構成をベースにしていますが、マスターゲインの代わりに双極の `Feedback Amount` スライダーを持ちます。このスライダーは単なる全体音量ではなく、EQ 出力の一部を EQ 入力へ戻す量と極性を決めます。
...
- 8 バンドの周波数は `EQ8` と共通で、`60Hz`、`170Hz`、`310Hz`、`600Hz`、`1kHz`、`3kHz`、`6kHz`、`12kHz` です。
...
- **その他の詳細**: パネル配置は EQ8 と同じで、上側に高域、下側に低域があります。
...
- **通常の EQ として使う**: `Feedback Amount = 0%` にして、`EQ8` と同じ固定 8 バンド EQ として使います。
